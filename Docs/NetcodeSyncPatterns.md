...
| 권한은 열되 특정 대상만 통과 | 호출은 `Everyone`/`RequireOwnership=false`로 개방 + 서버·수신부에서 `clientId`/팀/역할 대조 |
...
### 4-5. 권한 개방 후 신원 분기 (open-then-verify)

기본 RPC 권한은 소유자로 제한되지만, 이 프로젝트엔 **호출은 `Everyone`/`RequireOwnership=false`로 넓게 열고, 실제 반영은 서버·수신부에서 `clientId`·팀·역할로 좁히는** 사례가 반복된다. 단순 `IsServer`/`IsClient` 게이트가 아니라 *특정 개인·팀·역할만* 통과시키는 2단 구조다.

```
[호출 개방]  RequireOwnership=false / InvokePermission.Everyone   ← 누구나 호출 가능
      │
[신원 분기]  서버·수신부에서 대조:
      ├─ gunnerId == _gunnerId          → 그 탱크의 포수만 (터렛 조준)
      ├─ teamNum == 내팀 && role==Driver → 그 팀 운전수만 (HP 감산·시야효과)
      └─ TryGetValue(team)              → 그 팀 오디오소스만 (사운드 라우팅)
```

| RPC | 개방 | 통과 조건 |
| --- | --- | --- |
| `VehicleTurret.SendInputDataToServerRpc` | `RequireOwnership=false` | 서버가 `gunnerId == _gunnerId` → 정당한 포수만 터렛 반영 |
| `TankController.TakeDamageClientRpc` | ClientRpc(전원 수신) | `teamNum==내팀 && role==Driver` → 그 팀 운전수만 HP 감산 |
| `TankController.ViewEffectControlClientRpc` | ClientRpc | `teamNum==내팀 && role` → 각자 자기 팀·역할 효과만 on |
| `AudioManager.PlaySfxClientRpc` 등 | `Everyone` | `_playerSfxClips.TryGetValue(team)` → 그 팀 오디오소스로만 라우팅 |

> "호출 권한을 여느냐"와 "반영을 허용하느냐"를 분리한 것이 핵심이다. Netcode의 소유권 게이트만으로는 표현할 수 없는 *비소유자의 정당한 조작*(포수 조준)·*수신자별 선택 반영*(자기 팀 것만)을, 권한을 연 뒤 서버·수신부에서 신원을 대조해 구현한다. 개방과 검증을 나눠 유연성과 안전성을 동시에 얻는다.
...
### 6-5. 권한 개방 + 신원 대조 — 비소유 포수의 정당한 조작
...
// 반영은 신원 대조로 좁힘 (서버 수신부)
public void UpdateTurretPosition(Vector2 input, ulong gunnerId)
    { if (gunnerId == _gunnerId) _trlastInput = input; }   // 그 탱크의 포수만
```

> 포수는 탱크 소유자가 아니라 소유권 게이트를 통과하지 못하지만, 조준은 정당한 조작이다. `RequireOwnership=false`로 문을 열고 서버가 `gunnerId`로 신원을 확인해, 엉뚱한 클라의 입력은 걸러내면서 포수의 조작만 반영한다([`CoopTankControl`](./CoopTankControl.md) §6-2와 짝).
...
- **개방과 검증의 분리 (open-then-verify)** — Netcode의 소유권 게이트는 "소유자냐 아니냐"만 가른다. 이 프로젝트는 호출을 `Everyone`/`RequireOwnership=false`로 연 뒤, 서버·수신부에서 `clientId`·팀·역할을 대조해 *특정 개인·팀·역할만* 통과시킨다(§4-5). 비소유자의 정당한 조작(포수 조준)과 수신자별 선택 반영(자기 팀 사운드·효과)을, 단순 Client/Server 이분법을 넘어 세밀한 권한 분기로 구현한 지점.
